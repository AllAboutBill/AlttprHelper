...
MSU files should separated into folders containing PCM files. The app will add a dummy .msu file if one is not preset. The app will copy all pcm files, rename them to coincide with the seed name and place all files together in a new folder within your provided output path.
🛠 Using the Application
1. Customize Patch
...
Click on the "Create Custom Preset" button to open the preset dialog.
...
You are also able to save a yaml file directly from Alttpr.com/Customizer if you would prefer, or if there are advanced settings you would like to manage.
Editing a Preset:
...
🛠 Troubleshooting
Application Crashes When Playing Audio Without MSU Files:

Issue:
If the "Preview MSU" checkbox is checked and you attempt to play audio without any MSU files present, the application may crash.

Solution:

Ensure that MSU-1 soundpacks are correctly installed in the designated folder.
Verify the MSU folder path in the application's settings.
If no MSU files are available, uncheck the "Preview MSU" option before attempting to play audio.
GIF Popup Not Displaying Correctly:

Issue:
The animated GIF popup is not displaying at the desired size of 192x192 pixels.

Solution:

Ensure that your system supports the display size.
Verify that the selected sprite has an available GIF file.
Restart the application to reset the visualizer settings.
No MSU Files Found:

Issue:
The application fails to locate MSU files, preventing audio playback.

Solution:

Confirm that MSU-1 soundpacks are installed in the correct directory.
Use the "Customize Patch" dialog to reselect or update the MSU pack.
Check for any error messages or notifications within the application for further guidance.
🆘 Support
